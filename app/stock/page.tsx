...
export default function StockPage() {
  ...
  const canAdjust = can("stock", "edit");
  ...
  return (
    <section>
      ...
      <div className="panel">
        ...
        <table>
          <thead>
            <tr><th>{t("medicine")}</th><th>{t("sku")}</th><th>{t("batch")}</th><th>{t("qty")}</th><th>{t("expiry")}</th><th>{t("status")}</th>{canAdjust && <th></th>}</tr>
          </thead>
          <tbody>
            {visible.map((b) => {
              ...
              return (
                <tr key={b.id}>
                  ...
                  {canAdjust && <td><button className="ghost" onClick={() => adjust(b)}>{t("adjustAction")}</button></td>}
                </tr>
              );
            })}
          </tbody>
