...
export default function SuppliersPage() {
  ...
  const load = () => fetch("/api/suppliers?q=" + encodeURIComponent(q)).then((r) => (r.ok ? r.json() : [])).then(setRows);
  const loadPayments = () => fetch("/api/supplier-payments").then((r) => (r.ok ? r.json() : [])).then(setPayments).catch(() => {});
  useEffect(() => {
    load();
            ...
            {rows.map((s) => (
              <tr key={s.id}>
                ...
                <td className="rowActions">
                  {canEdit && s.dueAmount > 0 && <button className="ghost" onClick={() => pay(s)}>{t("payBtn")}</button>}
                  {canDelete && <button className="ghost danger" onClick={() => remove(s.id)}>{t("delete")}</button>}
                </td>
              </tr>
            ))}
          </tbody>
