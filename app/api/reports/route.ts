...
function periodKey(d: Date, groupBy: "day" | "month") {
...
export async function GET(req: Request) {
  ...
  // Default range: last 30 days (day view) or last 12 months (month view) when nothing is specified.
  const now = new Date();
  let rangeFrom: Date;
  ...
  return NextResponse.json({
    ...
    salesByPeriod: [...salesByPeriod.values()].sort((a, b) => a.period.localeCompare(b.period)),
