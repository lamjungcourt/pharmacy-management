import {NextResponse} from "next/server"; import {db} from "@/lib/prisma"; import {requirePermission,omitCostFields,canSeeProfit} from "@/lib/authz"; import {apiError} from "@/lib/api-error"; import type {Prisma} from "@prisma/client";

export async function GET(req: Request) {
  try {
    const ctx = await requirePermission("salesHistory", "view");
    ...
    // Each sale item stores the buy rate at time of sale (selling rate - buy rate = profit), so strip it
    // unless this user is allowed to see profit.
    return NextResponse.json(canSeeProfit(ctx.permissions) ? sales : omitCostFields(sales));
  } catch (e) {
    return apiError(e);
  }
}
...
export async function POST(req:Request){
 try{
 const ctx=await requirePermission("billing","add"); const session=ctx.user;
 ...
 return NextResponse.json(canSeeProfit(ctx.permissions)?result:omitCostFields(result),{status:201});
