...
export async function POST(req:Request){
 ...
 const b=await req.json(); const result=await db.$transaction(async (tx: Prisma.TransactionClient)=>{
   ...
   const discount=Number(b.discount??0), vatRate=settings?.vatRate??0, tax=(subtotal-discount)*vatRate/100, total=subtotal-discount+tax, paid=Number(b.paid??total);
   ...
   if(paid<total){
     ...
     await tx.customer.update({where:{id:customerId},data:{dueAmount:{increment:total-paid}}});
   }
