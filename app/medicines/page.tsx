...
export default function Medicines() {
  ...
  const { can, canSeeCost } = usePermissions();
  const canAdd = can("medicines", "add");
  const canDelete = can("medicines", "delete");
  ...
  const load = () => fetch("/api/medicines?q=" + encodeURIComponent(q)).then((r) => (r.ok ? r.json() : [])).then(setRows);
  useEffect(() => { load(); }, [q]);
    ...
    <section>
      ...
      <div className="panel">
        <table>
          <thead>
            <tr>
              <th>{t("brandName")}</th><th>{t("genericName")}</th><th>{t("type")}</th><th>{t("sku")}</th><th>{t("batch")}</th><th>{t("expiry")}</th><th>{t("qty")}</th><th>{t("freeQuantity")}</th>{canSeeCost && <th>{t("buyRate")}</th>}<th>{t("mrp")}</th>{canSeeCost && <th>{t("totalAmount")}</th>}<th>{t("status")}</th>{canDelete && <th></th>}
            </tr>
          </thead>
          <tbody>
            {rows.flatMap((m) =>
              m.batches.map((b: any) => (
                <tr key={b.id}>
                  ...
                  {canSeeCost && <td>{digits(b.purchaseRate)}</td>}
                  <td>{digits(b.sellingRate)}</td>
                  {canSeeCost && <td>{digits((b.quantity * b.purchaseRate).toFixed(2))}</td>}
