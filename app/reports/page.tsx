...
type Report = {
  ...
  summary: {
    billCount: number; totalRevenue: number; totalDiscount: number; totalTax: number; totalProfit?: number; // only sent to users allowed to see profit
...
export default function ReportsPage() {
  ...
  const load = () => {
    ...
    fetch("/api/reports?" + params.toString()).then((res) => (res.ok ? res.json() : null)).then((j) => j && setR(j));
  };
  ...
  return (
    <section>
      ...
      <div className="cards">
        ...
        {r.summary.totalProfit !== undefined && (
